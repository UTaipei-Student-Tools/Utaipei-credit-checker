...
def render_report(student_info, courses, report, major_domain, program_type, target_dept, offline_demo):
    summary = report["summary"]
    mode_tag = (
        " <span style='font-size:14px; background-color:#5c56e7; padding:2px 8px; border-radius:50px;'>離線 Demo 模式</span>"
        if offline_demo
        else " <span style='font-size:14px; background-color:#00cd98; padding:2px 8px; border-radius:50px;'>實時抓取模式</span>"
    )
        ...
        <div class="info-flex">
            <div class="info-card">
                ...
                <div style="margin-top:10px; font-size:15px; color:#0f172a; line-height:1.7;">
                    姓名：<strong>{student_info['name']}</strong><br>
                    學號：<strong>{student_info['student_id']}</strong><br>
                    系所：<strong>{student_info['department']}</strong><br>
                    列印日期：<strong>{student_info.get('print_date','-')}</strong>
                </div>
            </div>
    ...
    _render_metric_cards(summary, report, major_domain, program_type, target_dept)
    st.markdown("<br><br>", unsafe_allow_html=True)
    _render_tabs(courses, report, summary, major_domain, program_type, target_dept)


def _render_metric_cards(summary, report, major_domain, program_type, target_dept):
    colors = ["#00cd98", "#4facfe", "#ffaa00", "#94a3b8", "#a855f7", "#f97316"]
    labels = [
        ("🎓 實得總學分", summary["total_completed"], 128.0, summary["total_ip"], colors[0]),
        ("🏫 校共同+通識", summary["common_completed"], 28.0, summary["common_ip"], colors[1]),
        ("🔬 主修系專門學分", summary["major_completed"], 85.0, summary["major_ip"], colors[2]),
        ("🔓 自由選修學分", summary["free_completed"], 15.0, summary["free_ip"], colors[3]),
        ("✨ 跨系所學分", summary["target_completed"], 40.0 if program_type == "雙主修" else 20.0, summary["target_ip"], colors[4]),
        ("🎽 體育修課", report["pe"]["semesters_completed"], report["pe"]["semesters_required"], report["pe"]["semesters_ip"], colors[5]),
    ]
    ...
    for idx, (label, completed, target, ip, color) in enumerate(labels):
        if label == "✨ 跨系所學分" and program_type == "單主修":
            html_str.append(
                ...
                f"<div style='font-size:14px; font-weight:700; color:#475569; margin-bottom:12px;'>🚫 無跨系修讀</div>"
                f"<div style='font-size:28px; font-weight:800; color:#94a3b8; margin-bottom:6px;'>N/A</div>"
                f"<div style='font-size:12px; color:#94a3b8;'>目前為單主修身份</div>"
                f"</div>"
            )
            continue
            
        total_with_ip = summary.get("total_with_ip", completed + ip) if idx == 0 else completed
        bottom_text = f"已得 {completed:g} / 修讀中 {ip:g} / 應修 {target:g}" if idx == 0 else f"應修 {target:g} / 修讀中 {ip:g}"
...
def _render_tabs(courses, report, summary, major_domain, program_type, target_dept):
    st.markdown("### 🎯 畢業進度總覽")
    
    major_target = 128.0
    major_completed = summary["total_completed"] - summary["target_completed"]
    major_ip = summary["total_ip"] - summary["target_ip"]
    
    target_req = 40.0 if program_type == "雙主修" else (20.0 if program_type == "輔系" else 0.0)
    target_completed = summary["target_completed"]
    target_ip = summary["target_ip"]
    
    total_target = major_target + target_req
    total_completed_all = summary["total_completed"]
    total_ip_all = summary["total_ip"]

    draw_premium_progress(f"🏆 總進度條（主修加{program_type}）", total_completed_all, total_target, ip=total_ip_all)
    st.markdown("<div style='height:8px;'></div>", unsafe_allow_html=True)
    draw_premium_progress("📚 主修", major_completed, major_target, ip=major_ip)
    ...
    with tabs[0]:
        _render_overview_tab(courses, summary, report, major_domain, program_type, target_dept)

    with tabs[1]:
        _render_common_section(report["common"], report["pe"])

    with tabs[2]:
        _render_dept_compulsory_section(report["major"])

    with tabs[3]:
        _render_domain_compulsory_section(report["major"], major_domain)

    with tabs[4]:
        _render_domain_elective_section(report["major"], major_domain)

    with tabs[5]:
        _render_other_elective_section(report["major"])

    with tabs[6]:
        _render_free_elective_section(report["free"])

    with tabs[7]:
        _render_target_tab(report, program_type, target_dept, summary)

    with tabs[8]:
        _render_export_tab(courses, report, summary)

    with tabs[9]:
        render_schedule_planner()


def _render_overview_tab(courses, summary, report, major_domain, program_type, target_dept):
    st.markdown("### 📊 畢業學分進度總覽")
    _render_overview_progress_cards(courses, summary, report, program_type, target_dept)
    st.markdown("---")
...
def _render_pe_section(pe_report):
    ...
    in_progress = pe_report["semesters_ip"]
    status = "✅ 已修滿" if completed >= required else f"已修 {completed} / {required} 學期"
    color = "#00cd98" if completed >= required else "#4facfe"
    ...
    if course_rows:
        st.write(f"<div class='table-container'>{pd.DataFrame(course_rows).to_html(escape=False, index=False)}</div>", unsafe_allow_html=True)
    else:
...
def _render_common_compulsory_section(common_report):
    ...
    for missing in common_report["compulsory_missing"]:
        rows.append({"科目名稱": missing["name"], "修課學年": "--", "學分": missing["credit"], "成績": "--", "狀態": "缺漏", "狀態類別": "missing"})
    _render_course_cards(rows)
    _render_ge_categories(common_report)
    _render_common_elective_section(common_report)


def _render_ge_categories(common_report):
    st.markdown("### 🎨 通識分類選修領域")
    cols = st.columns(2)
    idx = 0
    for category_name, category_data in common_report["categories"].items():
        with cols[idx % 2]:
            status = "✅ 達標" if category_data["completed"] >= 4 else "🔵 修讀中" if category_data["completed"] + category_data["ip"] >= 4 else "⚠️ 未達標"
            color = "#00cd98" if category_data["completed"] >= 4 else "#4facfe" if category_data["completed"] + category_data["ip"] >= 4 else "#ff3860"
            st.markdown(
                f"""
                <div style='background: rgba(30, 41, 85, 0.2); border:1px solid rgba(255,255,255,0.05); border-radius:12px; padding:15px; margin-bottom:15px;'>
                    ...
                    <div style='font-size:13px; color:#cbd5e1; margin-bottom:10px;'>已得: {category_data['completed']:g} / 修讀中: {category_data['ip']:g} / 目標: 4 學分</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def _render_major_section_detailed(major_report, major_domain):
    ...
    _render_course_table_with_missing(major_report["domain_compulsory_courses"], major_report["domain_compulsory_missing"])
    st.markdown(f"**四、專業選修 (已取得 {major_report['domain_elective_completed']:g}/20 學分，修讀中 {major_report['domain_elective_ip']:g} 學分)**")
    _render_course_table(major_report["domain_elective_courses"])
    st.markdown(f"**五、其他本系課程 (已取得 {major_report['other_elective_completed']:g}/27 學分，修讀中 {major_report['other_elective_ip']:g} 學分)**")
    _render_course_table(major_report["other_elective_courses"])


def _render_common_section(common_report, pe_report):
    """展示校共同課程分類詳細"""
    st.markdown(f"### 一、校共同課程")
    total_completed = common_report['compulsory_completed'] + common_report['category_completed'] + common_report['common_elective_completed']
    total_ip = common_report.get('compulsory_ip', 0.0) + common_report.get('category_ip', 0.0) + common_report.get('common_elective_ip', 0.0)
    draw_premium_progress("校共同課程 (總計)", total_completed, 28, ip=total_ip)
    st.markdown(
        f"校共同必修 {common_report['compulsory_completed']:g}/10，通識分類選修 {common_report['category_completed']:g}/16，通識共同選修 {common_report['common_elective_completed']:g}/2"
    )
    st.markdown("---")
    ...
    # 通識
    st.markdown("#### 🎨 通識選修 (至少 4 科)")
    _render_ge_categories(common_report)
    _render_common_elective_section(common_report)


def _render_common_elective_section(common_report):
    st.markdown("#### 🧩 通識共同選修 (2 學分)")
    st.markdown(f"已取得 **{common_report['common_elective_completed']:g} / 2** 學分（修讀中 {common_report['common_elective_ip']:g}）")
    st.markdown("---")
...
def _render_dept_compulsory_section(major_report):
    """展示系共同必修"""
    st.markdown(f"### 二、系共同必修")
    draw_premium_progress("二、系共同必修", major_report['dept_compulsory_completed'], 24, ip=major_report['dept_compulsory_ip'])
    st.markdown("---")
...
def _render_domain_compulsory_section(major_report, major_domain):
    """展示專業必修"""
    st.markdown(f"### 三、專業必修 ({major_domain}領域)")
    draw_premium_progress(f"三、專業必修", major_report['domain_compulsory_completed'], 14, ip=major_report['domain_compulsory_ip'])
    st.markdown("---")
...
def _render_domain_elective_section(major_report, major_domain):
    """展示專業選修"""
    st.markdown(f"### 四、專業選修 ({major_domain}領域)")
    draw_premium_progress(f"四、專業選修", major_report['domain_elective_completed'], 20, ip=major_report['domain_elective_ip'])
    st.markdown("---")
...
def _render_other_elective_section(major_report):
    """展示其他本系課程"""
    st.markdown(f"### 五、其他本系課程")
    draw_premium_progress("五、其他本系課程", major_report['other_elective_completed'], 27, ip=major_report['other_elective_ip'])
    st.markdown("---")
...
def _render_free_elective_section(free_report):
    """展示自由選修"""
    st.markdown(f"### 六、自由選修")
    draw_premium_progress("六、自由選修", free_report['completed'], 15, ip=free_report['ip'])
    st.markdown("---")
...
def _render_target_tab(report, program_type, target_dept, summary):
    ...
    draw_premium_progress(f"🧪 {program_type} ({target_dept})", summary["target_completed"], target_req, ip=summary["target_ip"])
    st.markdown("---")
    
    if "物化系" in target_dept:
        st.markdown("#### 🅰️ 物化系共同基礎必修")
        _render_course_table_with_missing(report["target"]["basic_core_courses"], report["target"]["basic_core_missing"])
        ...
        _render_course_table_with_missing(report["target"]["compulsory_courses"], report["target"]["compulsory_missing"])
        st.markdown("#### 🅱️ 資科系其他選修")
        _render_course_table(report["target"]["elective_courses"])


def _render_export_tab(courses, report, summary):
    st.markdown(f"### 📦 六、自由選修 與 全部匯出 (已取得 {report['free']['completed']:g}/15 學分)")
...
def _render_parsed_course_totals(courses, summary, program_type):
    ...
    parsed_ip = sum((float(c.get('total_credit', 0.0) or 0.0) - float(c.get('completed_credit', 0.0) or 0.0)) if c.get('is_in_progress') else 0.0 for c in courses)
...
def _render_course_cards(rows):
    ...
    for r in rows:
        ...
        html.append(
            ...
            f"<div class='course-name-col' data-label='科目名稱'>{r['科目名稱']}</div>"
            f"<div data-label='修課學年' style='color:#334155;'>{r['修課學年']}</div>"
            f"<div data-label='學分' style='color:#334155;'>{r['學分']}</div>"
            f"<div data-label='成績' style='color:#334155;'>{r['成績']}</div>"
            f"<div class='course-status-col' data-label='狀態'><span style='display:inline-flex; align-items:center; justify-content:center; padding:8px 14px; border-radius:999px; background:{bg}; color:{color}; border:1px solid {border}; font-size:12px; font-weight:700;'>{status}</span></div>"
            f"</div>"
        )
