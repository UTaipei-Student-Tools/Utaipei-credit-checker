...
https://utaipei-credit-checker.streamlit.app/
...
Lieflat Charts 以可閱讀、可展開的 F1、F5、F7、F11 圖表呈現正式配置分布、畢業門檻、主修／輔系／雙主修判定與總學分進度。課程列數與要求項目數分開呈現；無法證明來源守恆、官方門檻或唯一配置時，圖表顯示不可用／待確認。圖表、明細與匯出都由相同 `DecisionSnapshot` 產生，並帶有同一 `snapshot_id` 與統計 digest。
