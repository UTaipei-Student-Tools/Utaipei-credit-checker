...
匯入後會先顯示可列印的課程表、讀取筆數與原始修得學分小計，供確認辨識結果；尚未確認或資料變更時，不會產生零學分畢業報告。確認通過後才計算畢業採計學分，並分別列出成績單實得學分、有效學分與尚未配置學分。
...
成績單課名前的通識標記（例如 `[通選公民]`）會保留於課程明細，比對時則使用去除已知標記後的課名。分類仍須符合該學期官方目錄；標記與官方分類衝突時不自動採認。
...
- 雙欄成績單依每段學年標題與跨欄、跨頁閱讀順序辨識學期；學年不明或衝突時停止正式採計。零學分體育仍保留為獨立修習紀錄。
