...
def parse_transcript_pdf(pdf_path):
    ...
    def merge_continuation_rows(rows, col_x=295):
        ...
        while i < len(rows):
            ...
            if left_text and not left_has_type and not left_has_credit and not any(term in left_text for term in ["學年", "累計學分", "實得學分", "附註"]) and i + 1 < len(rows):
                ...
                if (re.match(r"^[一二三四五六七八九十][\)\]]?$", next_left_text) or 
                    left_text.endswith("(") or 
                    (next_left_text and next_has_type)):
                    # 合併：目前行加上下一行左側的文字
            ...
            if right_text and not right_has_type and not right_has_credit and not any(term in right_text for term in ["學年", "累計學分", "實得學分", "附註"]) and i + 1 < len(rows):
                ...
                if (re.match(r"^[一二三四五六七八九十][\)\]]?$", next_right_text) or 
                    right_text.endswith("(") or 
                    (next_right_text and next_right_has_type)):
                    # 合併：目前行加上下一行右側的文字
    ...
    reported_total, reported_ip = extract_reported_totals_from_doc(all_pages)
    ...
    def parse_page_words(words, y_tol=3):
        ...
        exclude_contains = ["修習學分", "實得學分", "操行成績", "累計學分", "學年", "實得總學分數", "實得學分及平均成績", "抵免課程", "抵免學分", "科科科科", "修習總學分數", "附註", "[抵]", "[免]", "[未]", "[P]", "[F]", "[停]"]
        noise_patterns = [
            r"^\d+\.\d+\.\d+$",
            r"^\d+/\d+/\d+(?:\.\d+)?%?$",
            r"^\d+/\d+%$",
        ...
        for row in rows:
            ...
            if left_part:
                ...
                if (name and not any(term in name for term in exclude_contains) and not any(k in name for k in ["姓名", "系所", "畢業年月", "列印日期", "學號", "[WEB參考用]", "(大學部)"]) and not is_page_number and
                        is_valid_course_row(name, ctype, s1_cred, s1_score, s2_cred, s2_score, full_row_text)):
                    cd = build_course_dict(name, ctype, s1_cred, s1_score, s2_cred, s2_score, "113")
                    if cd:
                        parsed.append(cd)
            ...
            if right_part:
                ...
                if (name and not any(term in name for term in exclude_contains) and not any(k in name for k in ["姓名", "系所", "畢業年月", "列印日期", "學號", "[WEB參考用]", "(大學部)"]) and not is_page_number and
                        is_valid_course_row(name, ctype, s1_cred, s1_score, s2_cred, s2_score, full_row_text)):
                    cd = build_course_dict(name, ctype, s1_cred, s1_score, s2_cred, s2_score, "114")
                    if cd:
                        parsed.append(cd)
    ...
    parsed_total, parsed_ip = totals_from_courses(parsed_courses)
    ...
    if reported_total and parsed_total + 0.01 < reported_total:
        ...
        for ytol in (6, 9, 12, 18):
            ...
            alt_total, alt_ip = totals_from_courses(alt)
            if alt_total >= reported_total - 0.01:
                parsed_courses = alt
                parsed_total, parsed_ip = alt_total, alt_ip
                tried = True
                break
        if not tried:
            best = parsed_courses
