...
def _start_session(uid, pwd):
    ...
    try:
        ...
        pass
    response = session.post(LOGIN_URL, headers={**HEADERS, "Referer": BASE_URL + "index_main.html"}, data=data, timeout=15)
...
def _create_fnc_session(session, fncid):
    response = session.post(FNC_URL, headers={**HEADERS, "Referer": PERCHK_URL}, data={
        "fncid": fncid,
        "hid_type": "S",
        "hid_choice_check": "",
        "hid_teach_roll": "",
...
def discover_portal_features(uid, pwd):
    ...
    for fncid in ["AG104", "AG107", "AG108", "AG102"]:
        try:
            response = _create_fnc_session(session, fncid)
            if fncid == "AG104":
                available.append({"fncid": fncid, "name": "開課選課資料查詢 / 班級課表", "status": "可用"})
            elif fncid == "AG107":
                available.append({"fncid": fncid, "name": "教學評量查詢", "status": "可用"})
            elif fncid == "AG108":
                available.append({"fncid": fncid, "name": "教學評量填答率查詢", "status": "可用"})
            elif fncid == "AG102":
                available.append({"fncid": fncid, "name": "歷年成績單下載", "status": "可用"})
...
def crawl_course_schedule(uid, pwd, year='114', semester='2'):
    ...
    schedule_resp = _submit_portal_form(session, 'ag_pro/ag104.jsp?', form_data, BASE_URL + 'system/sys001_00.jsp?spath=ag_pro/ag104.jsp?')
    ...
    try:
        ...
        if 'application/pdf' in content_type:
            ...
            if not pdf_path_relative:
                raise ValueError("在成績頁面中找不到 PDF 下載連結。這可能是因為校務系統目前忙碌中，或者頁面結構已更新。請稍後再試。")

            if pdf_path_relative.startswith('//'):
                pdf_url = 'https:' + pdf_path_relative
            elif pdf_path_relative.startswith('http'):
                pdf_url = pdf_path_relative
            elif pdf_path_relative.startswith('/'):
                pdf_url = 'https://my.utaipei.edu.tw' + pdf_path_relative
            else:
                pdf_url = 'https://my.utaipei.edu.tw/' + pdf_path_relative.lstrip('/')
